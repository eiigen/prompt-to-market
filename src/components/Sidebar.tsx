...
export default function Sidebar({ onLoad }: Props) {
  ...
  return (
    <>
      {/* Hamburger */}
      <button onClick={() => setOpen(!open)} className="fixed top-5 left-4 z-[200] text-on-surface-variant hover:text-primary transition-colors p-2">
      ...
      {/* Overlay */}
      {open && <div onClick={() => setOpen(false)} className="fixed inset-0 z-[150] bg-black/40" />}

      {/* Panel */}
      <aside className={`fixed top-0 left-0 h-full z-[160] w-72 bg-surface-container-low border-r border-outline-variant transform transition-transform duration-200 ${open ? 'translate-x-0' : '-translate-x-full'} overflow-y-auto`}>
        <div className="p-lg pt-16">
          ...
          {history.length === 0 ? (
            ...
            <div className="flex flex-col gap-sm">
              {history.map((h) => (
                <div key={h.id} className="glass-card rounded-lg p-md group">
